...
class Contract(sp.Contract):
  def __init__(self):
    self.init_type(sp.TRecord(last_token_id = sp.TNat, ledger = sp.TBigMap(sp.TPair(sp.TAddress, sp.TNat), sp.TNat), metadata = sp.TBigMap(sp.TString, sp.TBytes), operators = sp.TBigMap(sp.TRecord(operator = sp.TAddress, owner = sp.TAddress, token_id = sp.TNat).layout(("owner", ("operator", "token_id"))), sp.TUnit), supply = sp.TBigMap(sp.TNat, sp.TNat), token_metadata = sp.TBigMap(sp.TNat, sp.TRecord(token_id = sp.TNat, token_info = sp.TMap(sp.TString, sp.TBytes)).layout(("token_id", "token_info")))).layout((("last_token_id", ("ledger", "metadata")), ("operators", ("supply", "token_metadata")))))
